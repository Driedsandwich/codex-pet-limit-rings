...
- One long-lived `codex app-server --stdio` connection using `account/rateLimits/read` plus sparse `account/rateLimits/updated` notifications for primary usage-limit data. A persistent monotonic watchdog reconciles 120 seconds after the last successful full snapshot; sparse notifications must not postpone that deadline. One five-second full read is allowed, sparse updates received in flight must be reapplied afterward, and an unresolved overdue read must invalidate the old connection generation before recreating the connection through bounded backoff. `Refresh Now` reuses a healthy, current connection and coalesces with its single in-flight read, but creates a fresh connection while disconnected, stale, or timed out.
- The `account/usage/read` method every 15 minutes for a memory-only, last-14-days daily usage view.
- `~/.codex/.codex-global-state.json` for `electron-avatar-overlay-open`, saved pet coordinates, and legacy `electron-avatar-overlay-bounds.mascot` metadata when present. Current ChatGPT builds may omit the top-level mascot size. Keep this file read-only, cache snapshots by device, inode, size, and nanosecond modification time, parse changed snapshots off the main thread, and use cached pet geometry to reject unrelated global clicks before any live refresh.
- `~/.codex/config.toml` for only the bounded `[desktop] avatar-overlay-mascot-width-px` setting when the modern oversized surface omits pet dimensions. Keep it read-only, cache it by the same file identity contract, ignore every other setting, and combine the width with the currently verified 192-by-208 pet canvas ratio.
...
Pet wakeups, size changes, and moves are driven by a filesystem watcher on `~/.codex/.codex-global-state.json`, official `com.openai.codex` application lifecycle notifications, and a persistent two-second dispatch watchdog for missed events. The fallback must not depend on a main-run-loop `Timer`, so an in-place ChatGPT update that replaces the process can hide stale rings and rediscover the new pet window without restarting the companion app. A matching on-screen Codex pet overlay is required before displaying rings; persisted open state and bounds are only a positioning reference. Legacy builds use the saved overlay geometry. Current builds that omit top-level mascot dimensions may use the exact on-screen `Codex Pet Mascot Effect` window, a small redacted layer-three direct pet surface, or the modern oversized transparent avatar panel owned by the running `com.openai.codex` process. The direct surface is valid only with tightly bounded size, aspect ratio, current-display containment, layer, on-screen state, and official process. The oversized panel additionally requires saved open state, screen-exceeding geometry, same-display containment, and tight saved-pet reference alignment; accept its exact generic `ChatGPT` title or permission-free redaction only with every other invariant, never as evidence alone. Reject normal ChatGPT windows, voice controls, Activity Stack, notifications, and unrelated processes. Named effect surfaces may derive current mascot size from the live surface and saved origin. The oversized panel is identity evidence only: align its pet frame from the saved origin plus the bounded desktop pet-width setting and current 192-by-208 canvas ratio, falling back to the bounded live-center reconstruction only when the setting is unavailable or invalid. Use the resulting pet bounds rather than an oversized panel for click and drag hit testing, and use historical size only when live geometry is missing or impossible. If macOS redacts the named effect window, require the official process, on-screen pet-effect layer, and bounded pet-relative geometry that still admits the maximum slider effect surface, without requesting a new permission. Do not treat generic `ChatGPT`, third-party wrapper, or Stage Manager windows as pet evidence. If ChatGPT exposes its bottom voice control, accept the normal surface or the exact named compact surface only when the official process, layer, centered position, and bounded geometry agree; keep a redacted-name fallback on the stricter normal-size minimum. Normalize a contracted surface around its live center to a bounded minimum interaction target, keep the visual rings circular on the click-through accessory panel, relocate readouts, and exclude only that target from drag tracking without capturing pixels or requesting a new permission. Hide the rings when Codex exits, the pet closes, or its overlay is minimized or on another Space, and restore them when the live overlay returns. Animate the effectively visible panel at a monotonic 10 frames per second and destroy the timer when the rings, live pet, or panel are not visible or Reduced Motion is enabled. Keep that live-window gate, event-driven path, size tracking, control exclusion, and drag mismatch protection intact when changing frame-following behavior.
...
SHA-256 shown above. That provenance artifact predates release-path sanitization.
Current and future artifacts must pass the local-path check, and locally
packaged candidates must pass the same core artifact verifier as downloaded
releases.
