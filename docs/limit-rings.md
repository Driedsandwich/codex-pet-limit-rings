...
The full `account/rateLimits/read` snapshot is decoded read-only. The app can display `rateLimitsByLimitId`, credit availability and balance, individual monthly spend control, limit-reached reason, and the available reset-credit count. The short-window bucket is optional: if Codex does not report it, the weekly and additional limits remain visible and the menu states that the short window was not reported. When Codex does report short-window usage, the menu separately states that enforcement status is not reported; the app does not infer an unlimited plan or permanent policy change. It never calls `account/rateLimitResetCredit/consume` and does not mutate the account.

Daily account usage has remained read-only since v0.7.0, is refreshed every 15 minutes, and is never persisted. Version 0.9.0 displays the stable response's aggregate current/longest streak, longest turn, peak-day, and lifetime fields. Version 1.0.0 adds only derived compatibility information: the bounded Codex CLI version, current source, separate in-memory observation times, freshness labels, and a small safe failure category. Version 1.0.1 adds bounded full-snapshot reconciliation and memory-only cadence observations. The published v1.0.2 release fixes that reconcile deadline so sparse notifications cannot postpone snapshot metadata refreshes, and labels full-snapshot freshness separately. Per-thread usage remains excluded: the app does not subscribe to `thread/tokenUsage/updated`, resume or fork threads, inspect prompts, retain thread identifiers, or parse SQLite/JSONL for usage.
