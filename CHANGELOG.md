...
- Cache the read-only desktop size setting by device, inode, size, and nanosecond modification time, and retain the v1.0.12 live-center reconstruction when the setting is absent or invalid.
- Keep the oversized window as identity evidence only while preserving strict surface rejection, pet-only mouse and drag hit testing, lifecycle visibility, controls, multiple displays, and permission-free privacy boundaries.
...
- Normalize a contracted control around its live center to a bounded minimum interaction target while keeping the visual rings circular on the click-through accessory panel, relocating hover readouts, and keeping control clicks out of pet-drag tracking.
- Preserve permission-free operation, size tracking, lifecycle visibility, drag following, and multi-display behavior.
...
- Pin both the long-term v1.0.0 provenance baseline and published v1.0.9 compatibility baseline by SHA-256 in CI, with current bundle, resource, privacy, and archive checks.
